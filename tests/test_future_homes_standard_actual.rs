use crate::common::{PROVIDED_EXPECTED_OUTPUT_DIR, TEMPORARY_OUTPUT_DIR, DEMO_FILES_DIR};
...
fn postproc_file_matches_expected(file_name: &str) {
    ...
    {
        for (actual_value, expected_value) in actual_record.as_ref()
            .unwrap()
            .iter()
            .zip(expected_record.unwrap().iter())
        {
                match (actual_value.parse::<f64>().ok(),  expected_value.parse::<f64>().ok()) {
                    (Some(actual_f64), Some(expected_f64)) => { 
                        assert_relative_eq!(actual_f64, expected_f64, max_relative = common::FLOAT_THRESHOLD);
                    }
